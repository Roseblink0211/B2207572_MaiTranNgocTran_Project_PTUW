...
export const validateBookForm = (book) => {
  ...
  if (!book.donGia || book.donGia <= 0)
    errors.donGia = "Đơn giá phải lớn hơn 0";

  // Số quyển phải >= 0
  if (!book.soQuyen || book.soQuyen < 0)
    errors.soQuyen = "Số quyển không hợp lệ";

  // Năm xuất bản phải >= 1900
  if (!book.namXuatBan || book.namXuatBan < 1900)
